...
const createMenu = (obj) => {
  const objeto = {
    fetchMenu: () => obj,
    consumption: arrEmpty,
    order: (string) => {
      objeto.consumption.push(string);
    },
    pay: () => {
      let sum = 0;
      const allConsum = objeto.consumption;
      for (let index = 0; index < allConsum.length; index += 1) {
        if (Object.keys(obj.food).includes(allConsum[index])) {
          sum += obj.food[allConsum[index]];
        }
        if (Object.keys(obj.drink).includes(allConsum[index])) {
          sum += obj.drink[allConsum[index]];
        }
      }
      return sum + (sum * 0.1);
    },
